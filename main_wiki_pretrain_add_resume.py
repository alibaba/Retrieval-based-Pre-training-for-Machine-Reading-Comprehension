...
def main():
    ...
    parser.add_argument("--resume", default=-1, type=int, help="The checkpoint index from resuming.")
    ...
    if args.pretrain is not None:
        ...
        if args.checkpoint != -1:
            checkpoint_dir = os.path.join(args.output_dir, f"checkpoint-{args.resume}/")
            model = ModelClass.from_pretrained(checkpoint_dir, **get_added_args(args, ModelClass))
            logger.warn(f"Loaded model state dict from checkpoint {checkpoint_dir}")
        else:
            model = ModelClass.from_pretrained(args.model_name_or_path, **get_added_args(args, ModelClass))
    ...
    if args.do_train or args.do_label:
        ...
        for train_file in input_file_list:
            ...
            logger.info(f' Num features from {train_file}: {step_num_map[train_file]}.')
    ...
    if args.resume != -1:
        ...
        scaler.load_state_dict(
            torch.load(os.path.join(checkpoint_dir, "scaler.pt"))
        )
        logger.warn(f"Loaded state dict for optimizer, scheduler and grad scaler from {checkpoint_dir}.")

    # Prepare data
    eval_examples = data_reader.read(input_file=args.predict_file)
    eval_tensors = data_reader.convert_examples_to_tensors(examples=eval_examples, tokenizer=tokenizer)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for cached_train_features_file in input_files:
                ...
                for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", dynamic_ncols=True)):
                    ...
                    if args.resume != -1 and global_step < args.resume:
