...
from general_util.logger import setting_logger, setting_logger_pai
from general_util.mixin import PredictionMixin
from models import model_map, get_added_args
from oss_utils import torch_save_to_oss, set_bucket_dir, load_buffer_from_oss
...
def train(args, train_dataset, model, tokenizer, processor):
    ...
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter(f'{args.output_dir}/runs/')
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0]:
                    ...
                    if global_step % 5 == 0:
                        logger.info(f"Global step: {global_step}\tTraining loss: {tr_loss / global_step}\t"
...
def evaluate(args, model, tokenizer, processor, prefix="", if_eval=True):
    ...
    for eval_output_dir in eval_outputs_dirs:
        ...
        out_label_ids = None
        indexes = []
        for batch in tqdm(eval_dataloader, desc="Evaluating", dynamic_ncols=True, disable=True):
            model.eval()
            inputs = processor.generate_inputs(batch, args.device)
...
def load_and_cache_examples(args, tokenizer, processor, evaluate=0):
    ...
    file_name = cached_features_file.split("/")[-1]
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        loaded_from_oss = False
        
        if args.oss_cache_dir is not None:
            try:
                examples, tensors = torch.load(load_buffer_from_oss(os.path.join(args.oss_cache_dir, file_name)))
                loaded_from_oss = True
            except:
                logger.info(f"Haven't found {file_name} in {args.oss_cache_dir}.")

        if not loaded_from_oss:
            ...
            if args.local_rank in [-1, 0]:
                if args.oss_cache_dir is not None:
                    oss_file_name = os.path.join(args.oss_cache_dir, file_name)
                    logger.info("Saving examples and tensors into oss %s", oss_file_name)
                    torch_save_to_oss((examples, tensors), oss_file_name)
                else:
...
def main():
    ...
    # Json config file, which will overrite the other parameters
    ...
    global logger
    logger = setting_logger_pai(os.path.join(args.output_dir, 'log_dir/'), args.local_rank)
    logger.warning(
    ...
    if args.local_rank in [-1, 0] and args.oss_cache_dir is not None:
        # set_bucket_dir(args.oss_cache_dir)
        torch_save_to_oss(args, os.path.join(args.oss_cache_dir, "training_args.bin"))
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = ModelClass.from_pretrained(checkpoint, **get_added_args(args, ModelClass))
            model.to(args.device)
