...
class WikiEntityPreTrainProcessorMP(object):
    ...
    def convert_examples_to_tensors(self, examples: List, tokenizer: PreTrainedTokenizer):
        ...
        if if_add_lm:
            logger.warning("Please use `processors/wiki_convert_examples_ent_mlm.py` to generate features with MLM.")
    ...
    def data_to_tensors(all_features: List[Dict]):
        ...
        return all_input_ids, all_token_type_ids, all_attention_mask, all_sentence_spans, \
            all_answers, all_pre_answers, all_feature_index
    ...
    def collate_fn(self, batch):
        ...
        if not self.add_lm:
            ...
            nn_answers = pp_answers = mlm_ids = None
        else:
            if len(batch[0]) == 11:
                return self.collate_fn_10(batch)
            else:
                raise NotImplementedError()
        ...
        outputs = (
            input_ids, token_type_ids, attention_mask, sentence_index,
            sentence_mask, sent_word_mask, answers, pre_answers
        )

        if self.add_lm:
            mlm_lds = torch.stack(mlm_ids, dim=0)
            outputs = outputs + (mlm_ids,)
        
        outputs = outputs + (feature_index,)

        return outputs

    @staticmethod
    def collate_fn_10(batch):
        # Just process sentence spans using multi-processing.
        input_ids, token_type_ids, attention_mask, sentence_spans, \
            answers, pre_answers, nn_answers, pp_answers, mlm_ids, \
            true_sent_ids, feature_index = list(zip(*batch))
        ...
        true_sent_ids = torch.stack(true_sent_ids, dim=0)
        feature_index = torch.stack(feature_index, dim=0)
        sentence_spans = torch.stack(sentence_spans, dim=0)

        is_query = (answers != -1) | (pre_answers != -1)
        max_query_num = is_query.sum(dim=1).max().item()
        query_num = is_query.sum(dim=1, keepdim=True)
        # max_query_num = (answers > -1).sum(dim=1).max().item()
        
        answers = answers[:, :max_query_num]
        # query_num = (answers > -1).sum(dim=1, keepdim=True)
        pre_answers = pre_answers[:, :max_query_num]
        ...
        max_sent_num = (sentence_spans[:, :, 0] > -1).sum(dim=1).max().item()
        sentence_spans = sentence_spans[:, :max_sent_num]
        true_sent_ids = true_sent_ids[:, :max_sent_num]
        query_mask = true_sent_ids < query_num
        ...
        reverse_sentence_index = torch.zeros(input_ids.size(), dtype=torch.long)
        ...
        for b in range(batch):
            for sid, span in enumerate(sentence_spans[b]):
                ...
                sentence_mask[b, sid] = 0
                sent_word_mask[b, sid, :lens] = torch.zeros(lens)
                reverse_sentence_index[b, s: (e + 1)] = torch.LongTensor([sid] * lens)

        query_token_mask = reverse_sentence_index < query_num
        assert torch.all(mlm_ids[~query_token_mask] == -1)
        reverse_sentence_index[~query_token_mask] = 0

        max_query_token_num = query_token_mask.sum(dim=1).max().item()
        mlm_ids = mlm_ids[:, :max_query_token_num]
        reverse_sentence_index = reverse_sentence_index[:, :max_query_token_num]

        outptus = (
            input_ids, token_type_ids, attention_mask, sentence_index, sentence_mask,
            sent_word_mask, answers, pre_answers, nn_answers, pp_answers, true_sent_ids,
            mlm_ids, reverse_sentence_index, feature_index
        )
    ...
    def generate_inputs(self, batch, device):
        ...
        if len(batch) == 14:
            # inputs["nn_answers"] = batch[8].to(device)
            # inputs["pp_answers"] = batch[9].to(device)
            inputs["true_sent_ids"] = batch[10].to(device)
            inputs["mlm_ids"] = batch[11].to(device)
            inputs["reverse_sentence_index"] = batch[12].to(device)
