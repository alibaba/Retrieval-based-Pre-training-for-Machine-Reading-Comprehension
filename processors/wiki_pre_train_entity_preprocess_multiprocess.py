...
def interface(instance):
    ...
    example = workflow_0(sentences, args.keep_prob, args.mask_prob, args.replace_prob)
...
pool = Pool(6, initializer=lambda: random.seed(args.seed))
pool.map(interface, tqdm(data, dynamic_ncols=True))
pool.close()
pool.join()

examples = [t for t in tqdm(examples)]

# print(examples.__class__)
print(len(examples))

r"""
It should be noted that the processed examples are different from 
"""
