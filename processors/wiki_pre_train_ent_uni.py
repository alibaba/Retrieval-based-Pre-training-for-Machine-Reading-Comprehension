...
class WikiEntityPreTrainUnifiedProcessor(object):
    r"""
    Unify:
    This processor use a single <mask> token to replace the nosied entities or nouns.
    """
    reader_name = 'wiki_pre_train_ent_unify'
    ...
    def convert_examples_to_tensors(self, examples: List, tokenizer: PreTrainedTokenizer):
        ...
        for (example_index, example) in tqdm(enumerate(examples), desc='Convert examples to features',
            ...
            for sent_id, q_sent in enumerate(example['question']):
                ...
                for _tuple in noise:
                    if len(_tuple) == 2:  # mask strategy
                        ...
                        # Unified mask token for multiple sentencepiece or wordpiece.
                        _mask_text = tokenizer.mask_token
                        ...
                        #     _replaced_text[2:-2]  # FIXME: Here is a bug for workflow 1/2/3
                        # _text = _text + _replaced_text  # A bug in pre-processing, remove the "\'", '{', '}' tokens.

                        # last_end = ent_e

                        # Discuss if replacement noise is needed. Since the replacement will
                        # help the position embedding be trained (longer sequence till 512).
                        # We don't process the selected to be replaced entity or word.
                        pass
                    else:
                        print(masked_ents)
                        print(replaced_ents)
                ...
                # The position of <mask> token should be marked
                # And a decoding sequence is needed.
                if if_add_lm:
            ...
            for sent_id, d_sent in enumerate(example['passage']):
                ...
                # FIXME: `d_text` is not used. The space is ignored.
                # sub_words = tokenizer.tokenize(d_sent['text'])
