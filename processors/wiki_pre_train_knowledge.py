...
class WikiKnowledgePreTrainProcessor(object):
    reader_name = 'wiki_pre_train_knowledge'
    ...
    def convert_examples_to_tensors(self, examples: List, tokenizer):
    ...
    def data_to_tensors(all_features: List[Dict]):
        ...
        return all_input_ids, all_segment_ids, all_input_mask, all_sentence_spans, all_answers, all_edge_index, all_edges, all_feature_index
    ...
    def generate_inputs(self, batch, device):
        ...
        edge_index = batch[5].to(device)  # [batch, max_edge_index_num]
        edges = batch[6].to(device)  # [batch, max_edge_index_num, max_edges_num]
