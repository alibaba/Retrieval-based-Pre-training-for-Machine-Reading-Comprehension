...
class WikiEntityPreTrainProcessorAR(object):
    reader_name = 'wiki_pre_train_ent_ar'
    ...
    def collate_fn(batch):
        ...
        input_ids, token_type_ids, attention_mask, sentence_spans, \
            true_sent_index, answers, feature_index = list(zip(*batch))
        ...
        true_sent_index = torch.stack(true_sent_index, dim=0)
        ...
        true_sent_index = true_sent_index[:, :max_sent_num]
        query_mask = true_sent_index < query_num
        ...
        return input_ids, token_type_ids, attention_mask, sentence_index, \
            sentence_mask, sent_word_mask, query_mask, true_sent_index, feature_index
    ...
    def generate_inputs(self, batch, device):
        ...
        query_mask = batch[6].to(device)
        true_sent_index = batch[7].to(device)
        # answers = batch[8].to(device)  # [batch, max_query_sent_num]
