...
class BertCQueryPreTrainedConfig(BertConfig):
    added_configs = [
        'query_dropout', 'cls_type', 'sr_query_dropout', 'lm_query_dropout', 'fp16_opt_level'
    ]

    def __init__(self, query_dropout=0.1, cls_type=0,
                 sr_query_dropout=0.1, lm_query_dropout=0.1,
                 fp16_opt_level='O1', **kwargs):
...
class ClsQueryBertModel(BertPreTrainedModel):
    config_class = BertCQueryPreTrainedConfig
    model_prefix = 'c_query_bert'

    def __init__(self, config: BertCQueryPreTrainedConfig):
        ...
        if config.fp16_opt_level == 'O2':
            self.c_query.layer_norm = FusedLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class ClsQueryBertModelForSRAndMLM(ClsQueryBertModel):
    model_prefix = 'c_query_bert_sr_mlm'
    ...
    def __init__(self, config: BertCQueryPreTrainedConfig):
        ...
        if config.fp16_opt_level == 'O2':
            self.sr_prediction_head.transform.LayerNorm = FusedLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
            self.lm_prediction_head.mlp_with_layer_norm.aLayerNorm = FusedLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
            self.lm_prediction_head.mlp_with_layer_norm.bLayerNorm = FusedLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class ClsQueryBertModelForMCRC(ClsQueryBertModel):
    model_prefix = 'c_query_bert_mcrc'
...
c_query_bert_models_map = {
    ClsQueryBertModelForSRAndMLM.model_prefix: ClsQueryBertModelForSRAndMLM,

    ClsQueryBertModelForMCRC.model_prefix: ClsQueryBertModelForMCRC
}
